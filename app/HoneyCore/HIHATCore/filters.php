...
class filters {
  ...
  public static function attackChecking( $strLeft, $strRight, $attacks_obj, $image = true) { 
      ...
      // echo '<pre>';print_r($strArrayLR);echo '</pre>';die('sublime_cek');
      foreach( $strArrayLR as $strkey => $strValue ) {
        if($strValue!=""){
            // check entire array           
          for( $i = 0; $i < $attacks_obj->getElementNr(); $i++ ) {    // check for each entry in Detection array                                     
                  if ( $attckListArray[ $i ][ 1 ] == true & $attckListArray[ $i ][ 2 ] == true ) {  // strict + case sensitive
                      ...
                      if ( !(strpos( $attckListArray[ $i ][ 0 ], $strValue ) === false) )   
                              array_push( $resultArr, $i );                                                                                              
                  }    
          }
        } 
      ...
      if ( count ( $resultArr ) < 1 )
          ...
          foreach ($resultArr as $key => $value) {
            $results[]= $attacks_obj->getStrVal( $value );
          }
          return $results;
